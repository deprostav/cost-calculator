...
function setEnergyClass(className) {
  ...
  document
    .querySelector(`.energy-class-${className.toLowerCase()}`)
...
function togglePhotovoltaicSlider() {
  const photovoltaicSliderContainer = document.getElementById(
    'photovoltaicSliderContainer',
  )
  photovoltaicSliderContainer.style.display = selectedMeasures.photovoltaic
    ? 'block'
    : 'none'
...
function toggleHeatPumpOptions() {
  ...
  if (heatPumpOptions) {
    ...
    if (
      !selectedMeasures.heatPump &&
      heatPumpTypeElement &&
      heatPumpTypeTextElement
    ) {
...
function toggleRainwaterTankIrrigation(option) {
  if (option === 'rainwaterTankIrrigation') {
    selectedMeasures.rainwaterTankIrrigation =
      !selectedMeasures.rainwaterTankIrrigation
    selectedMeasures.rainwaterTankIrrigationWC = false
  } else {
    selectedMeasures.rainwaterTankIrrigationWC =
      !selectedMeasures.rainwaterTankIrrigationWC
    selectedMeasures.rainwaterTankIrrigation = false
  }
  const rainwaterTankIrrigationButton = document.getElementById(
    'rainwaterTankIrrigationButton',
  )
  const rainwaterTankIrrigationWCButton = document.getElementById(
    'rainwaterTankIrrigationWCButton',
  )

  rainwaterTankIrrigationButton.classList.toggle(
    'selected',
    selectedMeasures.rainwaterTankIrrigation,
  )
  rainwaterTankIrrigationButton.classList.toggle(
  ...
  rainwaterTankIrrigationWCButton.classList.toggle(
    'selected',
    selectedMeasures.rainwaterTankIrrigationWC,
  )
  rainwaterTankIrrigationWCButton.classList.toggle(
...
function toggleEnergyMeasure(measure) {
  ...
  const button = document.getElementById(
    `uspory${measure.charAt(0).toUpperCase() + measure.slice(1)}Button`,
  )
...
function toggleCustomEnergyPriceFields() {
  const customEnergyPriceFields = document.getElementById(
    'customEnergyPriceFields',
  )
...
function toggleEnergyClassButtons() {
  const energyClassButtons = document.querySelector(
    '.button-group.energy-class',
  )
...
function calculateEnergyCost() {
  ...
  if (
    ...
    !isNaN(houseArea) &&
    document.querySelector('.button-group.energy-class').style.display ===
      'flex'
  ) {
  ...
  const electricityPrice =
    parseFloat(document.getElementById('electricityPrice').value) || 6338.95
  ...
  document.getElementById('monthlyEnergyCost').innerText = formatNumber(
...
function calculateSavings() {
  const monthlyEnergyCostText =
    document.getElementById('monthlyEnergyCost').innerText
  let monthlyEnergyCost =
    parseFloat(monthlyEnergyCostText.replace(/\s/g, '')) || 0

  let oldElectricityUsage =
  ...
  const photovoltaicKwp =
    photovoltaicSlider && selectedMeasures.photovoltaic
      ? parseFloat(photovoltaicSlider.value)
      : 0
  ...
  document.getElementById('finalMonthlyPayment').innerText =
    formatNumber(monthlyEnergyCost)
  document.getElementById('finalEnergyUsage').innerText = formatNumber(
    newTotalEnergyUsage * 1000,
  )

  const totalCo2Savings = (oldTotalEnergyUsage - newTotalEnergyUsage) * 1.17
  document.getElementById('totalCo2Savings').innerText =
    totalCo2Savings.toFixed(2)
...
function calculateTotalMonthlyCost() {
  const monthlyPayment =
    parseFloat(
      document.getElementById('monthlyPayment').innerText.replace(/\s/g, ''),
    ) || 0
  const finalMonthlyPayment = document.getElementById(
    'finalMonthlyPayment',
  ).innerText
  const monthlyEnergyCostAfterSavings =
    parseFloat(finalMonthlyPayment.replace(/\s/g, '')) || 0
...
function calculateTotalSavings() {
  ...
  const finalMonthlyCost = parseFloat(
    document.getElementById('totalMonthlyCost').innerText.replace(/\s/g, ''),
  )
...
function calculateSubsidy() {
  ...
  if (selectedMeasures.rainwaterTankIrrigation) {
    const tankVolume =
      parseFloat(document.getElementById('tankVolume').value) || 0
  ...
  const extensiveRoofArea =
    parseFloat(document.getElementById('extensiveRoofArea').value) || 0
  const semiIntensiveRoofArea =
    parseFloat(document.getElementById('semiIntensiveRoofArea').value) || 0
  const intensiveRoofArea =
    parseFloat(document.getElementById('intensiveRoofArea').value) || 0
  const steepRoofArea =
    parseFloat(document.getElementById('steepRoofArea').value) || 0
  greenRoofSubsidy += Math.min(
    extensiveRoofArea * 800 +
      semiIntensiveRoofArea * 900 +
      intensiveRoofArea * 1000 +
      steepRoofArea * 1000,
    100000,
  ) // Limit na zelenou střechu je 100 000 Kč

  // Kombinační bonus: 10 000 Kč za každé další opatření (mimo první)
  const selectedMeasuresCount =
    Object.values(selectedMeasures).filter(Boolean).length
  let combinationBonus =
    selectedMeasuresCount > 1 ? (selectedMeasuresCount - 1) * 10000 : 0

  // Bonus za děti (rodinný bonus)
  const numChildrenFullCare =
    parseFloat(document.getElementById('numChildrenFullCare').value) || 0
  const numChildrenSharedCare =
    parseFloat(document.getElementById('numChildrenSharedCare').value) || 0
  let familyBonus = numChildrenFullCare * 50000 + numChildrenSharedCare * 25000

  // Bonus za vymezenou obec
  const selectedRegionBonus = document.getElementById(
    'selectedRegionBonus',
  ).checked
  let regionBonus = 0
  ...
  document.getElementById('recommendedLoanAmount').innerText =
    formatNumber(estimatedBudget - totalSubsidy) + ' Kč'

  // Aktualizace na stránce nabídky
  if (document.getElementById('offerTotalSubsidy')) {
    document.getElementById('offerTotalSubsidy').innerText =
      formatNumber(totalSubsidy) + ' Kč'
    document.getElementById('offerEstimatedBudget').innerText =
      formatNumber(estimatedBudget) + ' Kč'
  }

  // Aktualizace jednotlivých dotací a bonusů na stránce nabídky
  document.getElementById('areaSubsidy').innerText =
    formatNumber(areaSubsidy) + ' Kč'
  document.getElementById('energySubsidy').innerText =
    formatNumber(energySubsidy) + ' Kč'
  document.getElementById('greenRoofSubsidy').innerText =
    formatNumber(greenRoofSubsidy) + ' Kč'
...
const measuresInfo = {
  heatPump: {
    ...
    description:
      'Tepelné čerpadlo snižuje náklady na vytápění a zvyšuje energetickou účinnost vašeho domu.',
  },
  ventilation: {
    name: 'Rekuperace vzduchu',
    icon: 'fa-solid fa-wind',
    description:
      'Rekuperace zajišťuje čerstvý vzduch v domě a snižuje tepelné ztráty větráním.',
  },
  waterRekuperation: {
    name: 'Rekuperace teplé vody',
    icon: 'fa-solid fa-water',
    description:
      'Rekuperace teplé vody umožňuje znovu využít teplo z odpadní vody a snížit spotřebu energie.',
  },
  photovoltaic: {
    name: 'Fotovoltaika',
    icon: 'fa-solid fa-solar-panel',
    description:
      'Instalace fotovoltaických panelů umožňuje vyrábět vlastní elektřinu ze sluneční energie.',
  },
  rainwaterTankIrrigation: {
    name: 'Nádrž na dešťovou vodu - Zálivka',
    icon: 'fa-solid fa-cloud-rain',
    description:
      'Sběr dešťové vody pro zálivku zahrady snižuje spotřebu pitné vody.',
  },
  rainwaterTankIrrigationWC: {
    name: 'Nádrž na dešťovou vodu - Zálivka + WC',
    icon: 'fa-solid fa-toilet',
    description:
      'Využití dešťové vody pro zálivku a splachování WC výrazně snižuje spotřebu pitné vody.',
  },
  chargingStation: {
    name: 'Elektro nabíječka',
    icon: 'fa-solid fa-charging-station',
    description:
      'Instalace nabíječky pro elektromobil podporuje využití ekologické dopravy.',
  },
  greenRoof: {
    name: 'Zelená střecha',
    icon: 'fa-solid fa-leaf',
    description:
      'Zelená střecha zlepšuje tepelnou izolaci a přispívá k lepšímu mikroklimatu.',
  },
}
...
function updateOfferPage() {
  ...
  document.getElementById('offerShadingArea').innerText =
    document.getElementById('shadingArea').value || '0'
  ...
  document.getElementById('offerEstimatedBudget').innerText =
    document.getElementById('estimatedBudget').innerText
  document.getElementById('offerTotalSavings').innerText =
  ...
  document.getElementById('offerMonthlyPayment').innerText =
    formattedMonthlyPayment

  // Délka financování a úroková sazba
  document.getElementById('offerLoanDuration').innerText = '25'
  document.getElementById('offerInterestRate').innerText = '3.19'
  ...
  if (!selectedMeasures.heatPump) {
    recommendedMeasuresText +=
      'Zvažte přidání tepelného čerpadla pro snížení nákladů na vytápění. '
  }
  if (!selectedMeasures.photovoltaic) {
    recommendedMeasuresText +=
      'Instalace fotovoltaiky by výrazně snížila Vaše účty za elektřinu. '
  }
  if (!selectedMeasures.insulation) {
    recommendedMeasuresText +=
      'Zateplení domu je zásadní pro snížení energetických ztrát. '
  }
  ...
  const totalSavings =
    parseFloat(
      document.getElementById('totalSavings').innerText.replace(/\s/g, ''),
    ) || 0

  const recommendedMeasuresList = document.getElementById(
  ...
  if (totalSavings < monthlyPayment) {
    ...
    recommendedMeasuresList.innerHTML =
      '<li>Vaše úspory jsou vyšší než splátky. Výborně!</li>'
  }
}
...
function prepareDataForRaynet() {
  ...
  const clientData = {
    ...
    clientSurname: document.getElementById('clientSurname')
      ...
      : '',
    clientBirth: document.getElementById('clientBirth')
    ...
    clientPhone: document.getElementById('clientPhone')
      ...
      : '',
    clientAIS: document.getElementById('clientAIS')
      ? document.getElementById('clientAIS').value
      : '',
    clientPermanentAddress: document.getElementById('clientPermanentAddress')
      ...
      : '',
    clientCorrespondenceAddress: document.getElementById(
  ...
  const propertyData = {
    ...
    electricityMeterNumber: document.getElementById('electricityMeterNumber')
      ...
      : '',
    eanCode: document.getElementById('eanCode')
      ? document.getElementById('eanCode').value
      : '',
    eicCode: document.getElementById('eicCode')
      ? document.getElementById('eicCode').value
      : '',
    ownershipType: document.getElementById('ownershipType')
  ...
  const calculationData = {
    ...
    floorArea: document.getElementById('floorArea')
      ...
      : '',
    shadingArea: document.getElementById('shadingArea')
      ? document.getElementById('shadingArea').value
      : '',
    ...
    electricityUsage: document.getElementById('electricityUsage')
      ? document.getElementById('electricityUsage').value
      : '',
    gasUsage: document.getElementById('gasUsage')
      ? document.getElementById('gasUsage').value
      : '',
    houseArea: document.getElementById('houseArea')
      ? document.getElementById('houseArea').value
      : '',
    heatPumpType: selectedMeasures.heatPump
      ? document.getElementById('heatPumpType').value
      : '',
    photovoltaic: selectedMeasures.photovoltaic
      ? document.getElementById('photovoltaicSlider').value
      : '',
    ...
    rainwaterTank: {
      ...
      type: selectedMeasures.rainwaterTankIrrigationWC
        ? 'Zálivka + WC'
        : 'Zálivka',
    },
    ...
    numChildrenSharedCare: document.getElementById('numChildrenSharedCare')
      ...
      : 0,
    // Celkový rozpočet
    totalBudget: document.getElementById('estimatedBudget')
      ? document.getElementById('estimatedBudget').innerText
      : '',
  ...
  if (selectedMeasures.heatPump) {
    ...
    heatPumpInfo = `Tepelné čerpadlo (${heatPumpType}) - Výše dotace: ${heatPumpSubsidy.toLocaleString(
      'cs-CZ',
    )} Kč.`
